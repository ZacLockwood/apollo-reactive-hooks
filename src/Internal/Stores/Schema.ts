...
export default class SchemaStore<TDomainStores = UntypedDomainStores> {
  ...
  constructor(schemaOptions: ApolloReactiveHooksSchemaOptions) {
  ...
  useDomainStore = <T extends Object>(typename: string) => {
    if (!this.domainStores[typename]) {
      throw new Error(
        `ApolloReactiveHooks could not find a registered class for type: '${typename}'.\nMake sure you've decorated the id field for this type's class with the @ApolloReactiveHooksID() annotation and that the class file is imported *before* ApolloReactiveHooksProvider.\nIf you'd prefer not to use annotations, you can alternatively add a type options definition to your Schema Options.`
      );
    }
