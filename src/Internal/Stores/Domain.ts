...
import {
  SchemaObjectValues,
  ServerSchemaObj,
  ApolloReactiveHooksDomainOptions,
} from "../Types";
...
export default class DomainStore<T extends Object> {
  ...
  constructor(
    typename: string,
    domainOptions: ApolloReactiveHooksDomainOptions
  ) {
    this.typename = typename;
    if (!domainOptions.idPropName) {
      throw new Error(
        `ApolloReactiveHooks was unable to resolve idPropName for type '${typename}'. Try adding an @ApolloReactiveHooksID() annotation to the id field for this class, or specify idPropName for '${typename}' type in your Schema Options.\n\nNote: When using annotations, you must import the annotated model class *before* importing ApolloReactiveHooksProvider or the annotations won't be picked up properly.`
      );
    }
    if (!domainOptions.mobxMode) {
      throw new Error(
        `ApolloReactiveHooks was unable to resolve mobxMode for type '${typename}'. Try adding an @ApolloReactiveHooksID({ mobxMode: [Your MobX Preference] }) annotation to the id field for this class, or specify mobxMode for '${typename}' type in your Schema Options.\n\nNote: When using annotations, you must import the annotated model class *before* importing ApolloReactiveHooksProvider or the annotations won't be picked up properly.`
      );
    }
    if (!domainOptions.objectFactory) {
      throw new Error(
        `ApolloReactiveHooks was unable to resolve objectFactory for type '${typename}'. Try adding an @ApolloReactiveHooksID() annotation to the id field for this class, or specify objectFactory for '${typename}' type in your Schema Options.\n\nNote: When using annotations, you must import the annotated model class *before* importing ApolloReactiveHooksProvider or the annotations won't be picked up properly.`
      );
    }
  ...
  writeObject = (serverData: ServerSchemaObj) => {
    ...
    const apolloReactiveHooksID = (domainObj as any)[this.idPropName];
    if (!apolloReactiveHooksID) {
      throw new Error(
        `ApolloReactiveHooksID property '${
          this.idPropName
        }'could not be retrieved from target:\n${JSON.stringify(
          domainObj,
          null,
          2
        )}\nPlease note that properties annotated with @ApolloReactiveHooksID or specified via idPropName must have an accessible value immediately upon object creation.`
      );
    }
    const observableRef = this.objectsDictionary[apolloReactiveHooksID];
    if (observableRef) {
      ...
      this.objectsDictionary[apolloReactiveHooksID] = domainObj;
    }
  };
}
